        ...
        for (int i = 0; i < regExp.size(); i++) {
            ...
            while (matcher.find()){
                switch (i) {
                    ...
                    case 1:
                        Double finish = Double.valueOf(matcher.group());
                        if (finish.equals(-0.0)) {
                            finish = Math.abs(finish);
                        }
                        System.out.println(String.format("%s %d", nf.format(finish),countOperation));
                        break;
                    case 2:
                        func = matcher.group().substring(0, 3);
                        switch (func) {
                            case "sin":
                                result = Math.sin(Math.toRadians(Double.valueOf(matcher.group().substring(3))));
                                break;
                            case "cos":
                                result = Math.cos(Math.toRadians(Double.valueOf(matcher.group().substring(3))));
                                break;
                            case "tan":
                                result = Math.tan(Math.toRadians(Double.valueOf(matcher.group().substring(3))));
                                break;
                        }
//                        clearExpression = matcher.replaceFirst(nf.format(result));
                        clearExpression = matcher.replaceFirst(String.format("%f", result));
                        recurse(clearExpression, countOperation);
                        return;
                    case 3:
                        if (matcher.group().contains("(")) {
                            firstArg = Double.valueOf(matcher.group().substring(1, matcher.group().indexOf(")")));
                        } else {
                            firstArg = Double.valueOf(matcher.group().substring(0, matcher.group().indexOf("^")));
                        }
                        secondArg = Double.valueOf(matcher.group().substring(matcher.group().indexOf("^") + 1));
...
//                        clearExpression = matcher.replaceFirst(nf.format(result));
                    ...
                    case 4:
                        ...
                        secondArg = Double.valueOf(matcher.group().substring(matcher.group().indexOf(func) + 1));
                        switch (func) {
                            case "*":
...
//                        clearExpression = matcher.replaceFirst(nf.format(result));
                    ...
                    case 5:
                        if (matcher.group().contains("+")){
                            ...
                            firstArg = Double.valueOf(matcher.group().substring(0, matcher.group().lastIndexOf("-")));
                            secondArg = Double.valueOf(matcher.group().substring(matcher.group().lastIndexOf("-") + 1));
                            result = firstArg - secondArg;
                        }
//                        clearExpression = matcher.replaceFirst(nf.format(result));
                        clearExpression = matcher.replaceFirst(String.format("%f", result));
